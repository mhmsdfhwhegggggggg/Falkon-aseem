...
// Credentials are validated lazily — the server starts without them so the
// web dashboard can load, but any Telegram operation will fail gracefully.
if (!API_ID || !API_HASH) {
  logger.warn("TELEGRAM_API_ID / TELEGRAM_API_HASH not set — Telegram operations will fail until configured");
}
